...
void diemDanh(DiemDanh &d){
    ...
    string ng,np,dl,dlnn,tt;
    ng="n";
    np="np";
    dl="dl";
    dlnn="dlnn";
    if(d.trangThaiDiLam==ng){
        tt="nghi";
    } else if(d.trangThaiDiLam==np)
    { tt="nghi phep";}
    else if (d.trangThaiDiLam==dl){
        tt="di lam";
    } else if (d.trangThaiDiLam==dlnn){
        tt="di lam nua ngay";
    }
    DSDD<<"," ;
    DSDD<<d.ngayDiemDanh;
